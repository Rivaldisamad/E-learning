...
if(@$_SESSION['admin'] || @$_SESSION['pengajar']) {
    ...
    <img src="img/LT.png" width="85" align="left">
    <style type="text/css">
    a.navbar-brand{
        margin-top: 13px
    }
    ...
    if(@$_SESSION['admin']) {
        ...
        <li>
        <a href="?page=daftarpengajar" class="<?php if(@$_GET['page'] == 'daftarpengajar') { echo 'active-menu'; } ?>"># Daftar pengajar</a>
        </li>
        <li>
        <a class="<?php if(@$_GET['page'] == 'kelas') { echo 'active-menu'; } ?>" href="?page=kelas"></i># Manajemen Kelas</a>
        </li>
        </ul>
        </li>
        <?php
    ...
    } else if(@$_GET['page'] == 'daftarpengajar') {
        include "inc/daftarpengajar.php";
    } else {
        echo "<div class='col-xs-12'><div class='alert alert-danger'>[404] Halaman tidak ditemukan! Silahkan pilih menu yang ada!</div></div>";
    } ?>
    
    <footer class="navbar-brand"><p> &copy; 2019 e-Learning SMA Taman Madya Jetis Yogyakarta | By : Rivaldi Samad</p></footer>
    </div>
    </div>
    </div>
    </body>
