...
$pdf->Image('../../style/assets/img/LT.png',263,18,17);
$pdf->SetTitle("Cetak Data");
$pdf->SetFont('Arial','B',18);
$pdf->Cell(0,5,'SMA Taman Madya Jetis Yogyakarta','0','1','C',false);
$pdf->SetFont('Arial','i',8);
$pdf->Cell(0,5,'Jl. Pakuningratan No.34A, 001, Cokrodiningratan, Jetis, Kota Yogyakarta, Daerah Istimewa Yogyakarta 55233','0','1','C',false);
...
if(@$_GET['data'] == "pengajar") {
	...
	while($data = mysqli_fetch_array($sql)) {
		...
		$pdf->Cell(25,4,$data['nip'],1,0,'C');
		$pdf->Cell(40,4,$data['nama_lengkap'],1,0,'C');
		$pdf->Cell(40,4,$data['tempat_lahir'].", ".tgl_indo($data['tgl_lahir']),1,0,'C');
		$pdf->Cell(23,4,$jk,1,0,'C');
		$pdf->Cell(20,4,$data['agama'],1,0,'C');
		$pdf->Cell(33,4,$data['jabatan'],1,0,'C');
		$pdf->Cell(52,4,$data['alamat'],1,0,'C');
	...
	while($data = mysqli_fetch_array($sql)) {
		...
		$pdf->SetFont('Arial','',7);
		$pdf->Cell(7,4,$no++.".",1,0,'C');
		$pdf->Cell(19,4,$data['nis'],1,0,'C');
		$pdf->Cell(32,4,$data['nama_lengkap'],1,0,'C');
		$pdf->Cell(36,4,$data['tempat_lahir'].", ".tgl_indo($data['tgl_lahir']),1,0,'C');
		$pdf->Cell(12,4,$data['jenis_kelamin'],1,0,'C');
		$pdf->Cell(17,4,$data['agama'],1,0,'C');
		$pdf->Cell(20,4,$data['nama_ayah'],1,0,'C');
		$pdf->Cell(20,4,$data['nama_ibu'],1,0,'C');
		$pdf->Cell(19,4,$data['no_telp'],1,0,'C');
		$pdf->Cell(35,4,$data['email'],1,0,'C');
		$pdf->Cell(38,4,$data['alamat'],1,0,'C');
		$pdf->Cell(10,4,$data['nama_kelas'],1,0,'C');
	}
	...
	while($data = mysqli_fetch_array($sql)) {
		...
		$pdf->Cell(60,4,$data['mapel'],1,0,'C');
	}
	...
	$no = 1;
	if(@$_SESSION[admin]) {
	    $sql_topik = mysqli_query($db, "SELECT * FROM tb_tugas_quiz JOIN tb_kelas ON tb_tugas_quiz.id_kelas = tb_kelas.id_kelas JOIN tb_mapel ON tb_tugas_quiz.id_mapel = tb_mapel.id ORDER BY tgl_buat ASC") or die ($db->error);
	} else if(@$_SESSION[pengajar]) {
	    $sql_topik = mysqli_query($db, "SELECT * FROM tb_tugas_quiz JOIN tb_kelas ON tb_tugas_quiz.id_kelas = tb_kelas.id_kelas JOIN tb_mapel ON tb_tugas_quiz.id_mapel = tb_mapel.id WHERE pembuat != 'admin' AND pembuat = '$_SESSION[pengajar]' ORDER BY tgl_buat ASC") or die ($db->error);
	} 
	...
	$id_tq = @$_GET['id_tq'];
	$sql_tq = mysqli_query($db, "SELECT * FROM tb_tugas_quiz JOIN tb_mapel ON tb_tugas_quiz.id_mapel = tb_mapel.id WHERE id_tq = '$id_tq'") or die($db->error);
	...
	$no = 1;
    $sql_nilai_pilgan = mysqli_query($db, "SELECT * FROM tb_nilai_pilgan JOIN tb_tugas_quiz ON tb_nilai_pilgan.id_tq = tb_tugas_quiz.id_tq JOIN tb_mapel ON tb_tugas_quiz.id_mapel = tb_mapel.id WHERE id_siswa = '$_SESSION[siswa]'") or die($db->error);
...
$pdf->Cell(0,0,"Yogyakarta, ".tgl_indo(date('Y-m-d')),0,0,'L');
